...
But someone has kindly assembled your genome for you using the `SPAdes` assembler (@Prjibelski2020-iq) and provided the result for you to download in two files, below - the assembled genome (`.fasta` file), and the assembly graph (`.gfa` file):
...
3. Under `Data structure for bins` select `In individual datasets` and choose the `Contigs` output from your `Shovill assembly`
4. Click on `Run Tool`
...
With the draft genome assembly you have for `ERR531380` you can use pubMLST to classify your isolate in more detail, using the `pubMLST` server.
