...
::: { .callout-challenge }
1. Visualise how your `ERR31380` reads map to the `ERR531413_asm.fasta` genome assembly
  - **NOTE:** you will need to map the reads to this genome using `BWA-MEM`, first
  - **NOTE:** you will need to set the BAM chunk size to `5500000`
2. Does this mapping differ from that to the _P. aeruginosa_ reference genome? Are there the same approximate number of SNPs? Do the same proportion of mapping reads carry the SNP?
:::
