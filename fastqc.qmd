...
1. Contamination: reads that derive from an organism we didn't intend to sequence; these are usually identified by querying against a database and subsequently removed
...
The `Falco` summary (@fig-falco-summary) provides an overview that indicates where there may be areas of concern in your data. Each section of the report receives a flag indicating either that it gets a _pass_ (<span style="color:green;">green</span>), a _warning_ (<span style="color:orange;">amber</span>), or a _fail_ (<span style="color:red;">red</span>) mark.
...
## Inspect the `Falco` output

::: { .callout-question }
What issues, if any, do you see in each read dataset?
:::
