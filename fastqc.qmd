...
The `Falco` summary (@fig-falco-summary) provides an overview that indicates where there may be areas of concern in your data. Each section of the report receives a flag indicating either that it gets a _pass_ (green, with a tick), a _warning_ (amber, with exclamation mark), or a _fail_ (red, with a cross) mark.
...
![Per base sequence quality score plot from `Falco` indicating high quality sequencing reads. Quality score ranges are presented as a boxplot at each sequence position range.](assets/images/falco_perbase.png){#fig-falco-perbase width=80%}
...
![Per sequence quality score plot from `Falco` indicating overall high quality sequencing reads.](assets/images/falco_perseq.png){#fig-fastqc-perseq width=80%}
