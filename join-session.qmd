...
::: { .callout-tip }
## You can use this link to log in to galaxy

- [{{< var galaxy.url >}}]({{< var galaxy.url >}})
:::

## The Galaxy workspace layout

Once you have logged in to Galaxy, you should see the workspace, which ought to look similar to @fig-galaxy-workspace. There are three sections to the workspace: the `Tools` sidebar, the `History/Workflow` sidebar, and the central `Workspace` area. You will become more familiar with these as you move through the workshop.
