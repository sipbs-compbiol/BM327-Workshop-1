...
::: { .callout-caution }
Data is not shared between Galaxy instances. For instance, if you upload data to `usegalaxy.eu` (Europe) it will _not_ automatically be available on `usegalaxy.org` (USA) or `usegalaxy.au` (Australia).
:::
...
When Galaxy begins to upload the files, the background changes to <span style="color:orange;">orange</span>.

When the upload is complete, the background turns <span style="color:green;">green</span>.

If there is a problem with uploading, the background turns <span style="color:red;">red</span> (please signal to a demonstrator if this happens for you).
:::
...
1. Drag the `ERR531413_asm.fasta` file into the Galaxy workspace.
...
1. Click on the file name (e.g. `GCF_000006765.1_ASM676v1_genomic.fna`). The file descriptor will enlarge.
...
1. Click on the `eye` icon of one of the file descriptors (e.g. `ERR531413_asm.fasta`). A representation of the data in the file will be shown in the workspace window.
